...
describe Pochette::Backends::BitcoinCashWrapper do
  let(:addresses){
    [
      'bchtest:qqkcrvssm6m7yfr4e4ljlkst7kpdmhya5udm4gne8z',
      'bchtest:qzhlqqgn4cekgcd2zcf74j2uxfum2ve8jvq7lsfnlf',
      'bchtest:qz5zujysvmxgzuk3474gfwt049sne5y42cfumfes4c',
      'bchtest:qzade0awfkp62twzermgvptx8twf6jfz5c7nxkhzm3',
      'bchtest:qrg50axjjgg9d2nd334907926ntg2gu4ncfd5ud36f',
      'bchtest:qqf7vqlqmx9ngrf2hslzv4fyn0d9chrvms55kxwmar',
    ]
  }
  ...
  it 'implements balances_for' do
    ...
    stub_rpc('listunspent', [0,99999999, legacy_addresses], 'balances_for_listunspent')
  ...
  it 'implements list_unspent' do
    stub_rpc('listunspent', [1, 99999999, legacy_addresses], 'list_unspent_listunspent')
  ...
  it 'implements block height' do
    stub_rpc('getblockcount', [], 'getblockcount')
    backend.block_height.should == 504457
  end

  let(:hex) { '' }
  it 'implements #propagate' do
    stub_rpc('sendrawtransaction', [], 'sendrawtransaction')
    backend.pushtx(hex).should == 'f5a5ce5988cc72b9b90e8d1d6c910cda53c88d2175177357cc2f2cf0899fbaad'
  end
end
